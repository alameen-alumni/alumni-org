...
export default function StepBasicInfo({ form, handleChange, alumniName, regIdExists, alreadyRegistered, isLoading, handleContinue }) {
  ...
  return (
    <>
      ...
      {/* Gender Field */}
      <div>
        <label className="block text-sm font-medium text-teal-700 mb-1.5" htmlFor="gender">Gender <span className="text-red-500">*</span></label>
        <select id="gender" name="gender" value={form.gender || ''} onChange={handleChange} required className="w-full pl-4 pr-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-transparent">
      ...
      <Button className="w-full mt-4" type="button" disabled={!regIdExists || !form.name || !form.gender || !form.event?.present || alreadyRegistered || (form.event?.present === 'yes' && !form.event?.coming_with_anyone) || (form.event?.coming_with_anyone === 'yes' && (!form.event?.accompany || !form.event?.accompany_rel))} onClick={handleContinue}>Continue</Button>
    </>
  );
} 
...
export default async function handler(req: any, res: any) {
  // Basic CORS and method handling to avoid preflight failures
  ...
  if (req.method === 'GET') {
    res.status(200).json({ ok: true, envConfigured: Boolean(process.env.RESEND_API_KEY), fromConfigured: Boolean(process.env.RESEND_FROM) });
    return;
  }
  if (req.method !== 'POST') {
